...
=item B<-S>|B<--use-system-bins>

Use system binaries instead of chroot's binaries. This might help if attempting
to run a chroot with a different architecture to the fakechroot library. If you
are going to be executing setuid binaries, you must set the B<--elfloader>
option to your architecture's dynamic linker, otherwise you will get elf class
errors.

Try this setting if you get a similar error to the following:
